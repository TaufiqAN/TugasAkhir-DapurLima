...
<body>
    <nav class="fixed top-4 left-1/2 transform -translate-x-1/2 w-[90%] md:w-[80%] bg-white border border-gray-200 shadow-lg rounded-full px-6 py-3 z-50">
        <div class="flex flex-wrap relative items-center justify-between">
            ...
            <div class="flex md:order-2 space-x-3 items-center">
                ...
                <div class="relative">
                    <img id="avatarButton" type="button" 
                        data-dropdown-toggle="userDropdown" class="w-12 h-12 rounded-full cursor-pointer" 
                        src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=random&color=fff&size=100" 
                        alt="User dropdown">
                ...
                @else
                <a href="{{ route('login') }}" class="text-center inline-flex items-center bg-[#1E811B] text-white rounded-full px-6 py-3 hover:bg-green-600">
                    <svg class="w-6 h-6 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                        <path fill-rule="evenodd" d="M12 20a7.966 7.966 0 0 1-5.002-1.756l.002.001v-.683c0-1.794 1.492-3.25 3.333-3.25h3.334c1.84 0 3.333 1.456 3.333 3.25v.683A7.966 7.966 0 0 1 12 20ZM2 12C2 6.477 6.477 2 12 2s10 4.477 10 10c0 5.5-4.44 9.963-9.932 10h-.138C6.438 21.962 2 17.5 2 12Zm10-5c-1.84 0-3.333 1.455-3.333 3.25S10.159 13.5 12 13.5c1.84 0 3.333-1.455 3.333-3.25S13.841 7 12 7Z" clip-rule="evenodd"/>
                        </svg>                                                   
                    <span class="hidden sm:inline ms-2">Masuk</span>
                </a>
                @endif
            </div>
            ...
            {{-- dropdown --}}
            <div id="navbar-cta" class="hidden md:flex md:order-1 lg:static absolute w-full md:w-auto top-12 mx-auto mt-4 bg-white rounded-lg shadow-md p-4 z-40 md:mt-0 md:bg-transparent md:shadow-none md:p-0">
                <ul class="flex flex-col md:flex-row md:space-x-8 font-medium">
                    <li>
                        <a href="{{ route('home') }}" class="block py-2 px-3 hover:text-blue-700">Beranda</a>
                    </li>
                    <li class="relative">
                                ...
                                @foreach($allKategori as $kategori)
                                    <li>
                                        <a href="{{ route('resep.semua_kategori', $kategori->slug) }}"
                                            class="block px-4 py-2 hover:bg-green-700 text-gray-800 hover:text-white">
                    ...
                    <li><a href="{{ route('resep.semua') }}" class="block py-2 px-3 hover:text-blue-700">Resep</a></li>
                    <li><a href="{{ route('tentang') }}" class="block py-2 px-3 hover:text-blue-700">Tentang</a></li>
                </ul>
            </div>
        </div>
    </nav>
