    ...
    <div id="reviewModal" class="z-50 fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center hidden px-4 overflow-y-auto">
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md my-10">
            ...
            <div class="flex items-center gap-4 overflow-hidden rounded-lg mt-2">
                ...
                <span class="font-medium text-base text-gray-700">{{ $resep->nama_resep }}</span>
            </div>
            
            <form id="reviewForm" method="POST" action="{{ route('reviews.store') }}" class="mt-4">
                @csrf
                ...
                <div class="mt-2"> 
                    ...
                    <div id="rating" class="flex space-x-1 cursor-pointer">
                        @for ($i = 1; $i <= 5; $i++)
                            <span class="star text-gray-300 text-4xl sm:text-5xl md:text-6xl" data-value="{{ $i }}">★</span>
                        @endfor
                    </div>
            ...
            <ul class="pt-4 mt-4 space-y-2 font-medium border-t dark:border-gray-600">
                ...
                @auth
                    @if (auth()->check() && (auth()->user()->role === 'admin' || auth()->id() === $review->user_id))
                        <form action="{{ route('admin.reviews.destroy', $review->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus ulasan ini?');" class="mt-2">
                            @csrf
                            @method('DELETE')
