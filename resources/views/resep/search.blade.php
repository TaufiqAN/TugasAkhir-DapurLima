...
@section('content')
    <div class="py-8 px-4 mx-auto max-w-6xl text-center mt-28">
        <h2 class="text-2xl font-bold">Hasil Pencarian: "{{ $keyword }}"</h2>
        @if ($resep->isEmpty()) 
        ...
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8">
                @foreach($resep as $search)
                    <div class="block group">    
                        <a href="{{ route('makanan.detail', $search->id) }}" class="block">
                            <div class="relative overflow-hidden rounded-lg">
                                <img src="{{ asset('storage/' . $search->image) }}" 
                                    alt="{{ $search->nama_resep }}"
                        ...
                        <div class="mt-4 text-gray-900 text-start group-hover:text-green-700 transition-all duration-300 flex justify-between items-center">
                            ...
                            <a href="{{ route('makanan.detail', $search->id) }}" class="text-lg font-semibold">
                                {{ $search->nama_resep }}
                            </a>
                        ...
                        <p class="text-sm flex items-center gap-2">
                            ...
                            <span class="flex items-center gap-1">
                                ...
                                <span>{{ $search->porsi }} Orang </span>
                            </span>
                            
        
                             {{-- Rating  --}}
                            <span class="flex items-center gap-1">
                                @if($search->total_rating)
                                    <span class="text-yellow-500 font-semibold text-sm">
                                        ★ {{ number_format($search->total_rating, 1) }} 
                                    </span>
                                @else
                                    <span class="text-yellow-500 font-semibold text-sm">
                                        ★ {{ number_format($search->total_rating, 1) }} 
