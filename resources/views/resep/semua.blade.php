...
<div class="max-w-6xl mx-auto py-24">
    ...
    {{-- Grid Card --}}
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-6 px-4 sm:px-0">
    @foreach ($allResep as $resep)
        <div class="block group">
                ...
                <div class="relative overflow-hidden rounded-lg">
                    <img src="{{ asset('storage/' . $resep->image) }}" 
                        ...
                        class="w-full h-52 object-cover rounded-lg brightness-75 transition-all duration-300 
                            sm:group-hover:brightness-100 sm:group-hover:scale-105">
                </div>
            </a>

            <div class="mt-4 flex justify-between items-start">
                {{-- Nama resep dengan link --}}
                <a href="{{ route('makanan.detail', $resep->id) }}" class="text-lg sm:text-xl font-semibold text-gray-900 group-hover:text-green-700 transition-all duration-300 min-h-[50px]">
                    {{ $resep->nama_resep }}
                </a>
                ...
                <button class="save-btn transition duration-300" data-resep-id="{{ $resep->id }}"
                    ...
                    @if ($isSaved)
                        <svg class="w-8 h-8 sm:w-8 sm:h-8 text-yellow-500" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" aria-label="Tersimpan">
                            <path d="M7.833 2c-.507 0-.98.216-1.318.576A1.92 1.92 0 0 0 6 3.89V21a1 1 0 0 0 1.625.78L12 18.28l4.375 3.5A1 1 0 0 0 18 21V3.889c0-.481-.178-.954-.515-1.313A1.808 1.808 0 0 0 16.167 2H7.833Z"/>
                        </svg>
                    @else
                        <svg class="w-8 h-8 sm:w-8 sm:h-8 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-label="Simpan">
            ...
            <p class="text-sm flex items-center gap-4 mt-2 flex-wrap">
                {{-- Ikon Waktu --}}
