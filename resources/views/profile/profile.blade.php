        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-6">
            @foreach ($savedResep as $save)
                ...
                <div class="block group">
                    <!-- Gambar dengan link -->
                    <a href="{{ route('makanan.detail', $save->resep->id) }}" class="block">
                        <div class="relative overflow-hidden rounded-lg">
                            <img src="{{ asset('storage/' . $save->resep->image) }}" 
                                alt="{{ $save->resep->nama_resep }}"
                    ...
                    <div class="mt-4 flex justify-between items-start">
                        ...
                        <a href="{{ route('makanan.detail', $save->resep->id) }}" class="text-xl font-semibold text-gray-900 group-hover:text-green-700 transition-all duration-300 min-h-[50px]">
                            {{ $save->resep->nama_resep }}
                        </a>
                        ...
                        @auth
                            ...
                            @endphp
                            <button class="save-btn transition duration-300" data-resep-id="{{ $save->resep->id }}">
                                @if ($isSaved)
                                    <!-- Sudah disimpan -->
                    ...
                    <p class="text-sm flex items-center gap-4 mt-2">
                        ...
                        <span class="flex items-center gap-1">
                            ...
                            <span>{{ $save->resep->waktu }} Menit</span>
                        </span>
