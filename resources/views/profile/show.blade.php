...
@section('content')
<div class="max-w-3xl mx-auto mt-36 px-4 sm:px-0">
    <div class="mb-10">
        <div class="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-4">
            <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=random&color=fff&size=100" 
                 alt="Avatar" class="w-24 h-24 sm:w-28 sm:h-28 rounded-full border-4 border-blue-500 shadow-lg">
            <div class="text-center sm:text-left">
                ...
                <div class="flex justify-center sm:justify-start mt-3">
                    <a href="/logout" class="flex items-center px-3 text-sm py-1 border-2 border-gray-800 text-gray-900 font-medium rounded-xl transition duration-300 hover:bg-gray-800 hover:text-white">
                        <svg class="w-5 h-5 sm:w-6 sm:h-6 me-2" fill="none" viewBox="0 0 24 24">
    ...
    {{-- List Tersimpan --}}
    <div class="bg-white shadow-md rounded-lg p-4 mt-6 mb-20 border">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 space-y-2 sm:space-y-0">
            <div class="flex items-center space-x-2">
                ...
                <h2 class="text-base sm:text-lg font-bold text-gray-800">Tersimpan</h2>
            </div>
            <a href="{{ route('profile.profile') }}" class="text-green-600 font-semibold text-sm hover:underline">Lihat Semua</a>
        </div>

        @if ($savedResep->isEmpty())
            <div class="flex flex-col items-center text-center text-gray-600 py-6">
        ...
        @else
            <div class="space-y-4">
                @foreach ($savedResep as $save)
                    @if ($save->resep)
                        <div class="flex flex-col sm:flex-row items-center sm:items-start rounded-lg border-2 p-4 gap-4">
                            <img src="{{ asset('storage/' . $save->resep->image) }}" alt="{{ $save->resep->nama_resep }}"
                                 class="w-full sm:w-44 h-44 sm:h-32 object-cover rounded-lg">
                            <div class="w-full sm:w-auto">
                                <h3 class="text-base sm:text-lg font-semibold text-gray-800">{{ $save->resep->nama_resep }}</h3>
                                <p class="text-sm text-gray-600 mb-3 sm:mb-6">{{ \Illuminate\Support\Str::limit($save->resep->deskripsi, 120, '...') }}</p>
                                <div class="flex flex-wrap gap-4 text-sm text-gray-700">
                                    <span class="flex items-center">
                                        <svg class="w-4 h-4 mr-1 text-green-700" fill="none" viewBox="0 0 24 24">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <span class="flex items-center">
                                        <svg class="w-4 h-4 mr-1 text-green-700" fill="currentColor" viewBox="0 0 24 24">
                                            <path fill-rule="evenodd" d="M8 4a4 4 0 1 0 0 8 4 4 0 0 0 0-8Zm-2 9a4 4 0 0 0-4 4v1a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2v-1a4 4 0 0 0-4-4H6Zm7.25-2.095c.478-.86.75-1.85.75-2.905a5.973 5.973 0 0 0-.75-2.906 4 4 0 1 1 0 5.811ZM15.466 20c.34-.588.535-1.271.535-2v-1a5.978 5.978 0 0 0-1.528-4H18a4 4 0 0 1 4 4v1a2 2 0 0 1-2 2h-4.535Z" clip-rule="evenodd"/>
                                        </svg>
                                        {{ $save->resep->porsi }} Orang
                                    </span>
                                    <span class="text-yellow-500 font-semibold">
                                        ★ {{ number_format($save->resep->total_rating ?? 0, 1) }}
                                    </span>
                                </div>
                            </div>
                        </div>
